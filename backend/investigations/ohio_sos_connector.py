...
Usage:
    ...
    results = search_entities("Do Good Ministries", records)
    ...
    results = search_entities("Do Good", all_records)
"""
    ...
    *,
    ...
    Two matching modes:
        Exact (default): case-insensitive substring match.
            "do good" matches "DO GOOD MINISTRIES INC"
        ...
        Fuzzy (opt-in): uses entity_normalization.normalize_org_name() to
            ...
            "Do Good Ministries" matching "DO GOOD MINISTRIES INC"
    ...
    Example:
        records = fetch_report(ReportType.NONPROFIT_CORPS)
        result = search_entities("Do Good Ministries", records)
...
# in a Catalyst investigation based on the founding Do Good Inc. case:
#   - Nonprofits (Do Good Ministries, Veterans Center)
#   - Domestic LLCs (Do Good RE LLC, Homan AG Management LLP)
...
def search_ohio(
    ...
    Example:
        result = search_ohio("Do Good Ministries")
        for match in result.matches:
