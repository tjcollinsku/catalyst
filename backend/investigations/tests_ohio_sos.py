...
202301010001,4567890,01/15/2023,EXAMPLE CHARITY MINISTRIES INC,,DOMESTIC ARTICLES/NON-PROFIT,EXAMPLE CHARITY MINISTRIES INC,123 MAIN ST,,CAREY,OH,43316,JOHN A EXAMPLE,456 FARM RD,,CAREY,OH,43316,CAREY,WYANDOT,JANE SMITH|JOHN A EXAMPLE
202301010002,4567891,01/18/2023,EXAMPLE CHARITY RE LLC,,DOMESTIC LLC,,,,FINDLAY,OH,45840,CAROL JONES,789 OAK AVE,,FINDLAY,OH,45840,FINDLAY,HANCOCK,CAROL JONES
...
202302010001,4567890,02/01/2023,EXAMPLE CHARITY MINISTRIES INC,AMENDED ARTICLES,EXAMPLE CHARITY MINISTRIES INC,123 MAIN ST,,CAREY,OH,43316,CAREY,WYANDOT
202302010002,4567891,02/05/2023,EXAMPLE CHARITY RE LLC,AMENDED ARTICLES,,,,FINDLAY,OH,45840,FINDLAY,HANCOCK
"""

# A CSV with one good row and one row with a missing BUSINESS NAME — should be skipped.
MALFORMED_CSV = """\
DOCUMENT NUMBER,CHARTER NUMBER,EFFECTIVE DATE,BUSINESS NAME,CONSENT FLAG,TRANSACTION CODE DESCRIPTION,FILING ADDRESS NAME,FILING ADDRESS 1,FILING ADDRESS 2,FILING CITY,FILING STATE,FILING ZIP,AGENT ADDRESS NAME,AGENT ADDRESS 1,AGENT ADDRESS 2,AGENT CITY,AGENT STATE,AGENT ZIP,BUSINESS CITY,COUNTY,BUSINESS ASSOCIATE NAMES
202301010001,4567890,01/15/2023,EXAMPLE CHARITY MINISTRIES INC,,DOMESTIC ARTICLES/NON-PROFIT,EXAMPLE CHARITY MINISTRIES INC,123 MAIN ST,,CAREY,OH,43316,JOHN A EXAMPLE,456 FARM RD,,CAREY,OH,43316,CAREY,WYANDOT,
    ...
    def _make_records(self):
        ...
        return [
            EntityRecord(
                ...
                business_name="EXAMPLE CHARITY MINISTRIES INC",
                ...
                downloaded_at=ts,
                statutory_agent="JOHN A EXAMPLE",
                agent_city="CAREY",
                agent_state="OH",
            ),
            EntityRecord(
                ...
                business_name="EXAMPLE CHARITY RE LLC",
...
class SearchEntitiesFuzzyTests(unittest.TestCase):
    ...
    def test_fuzzy_strips_legal_designators_before_match(self):
        ...
        name before comparing. A query of "Example Charity Ministries" (without INC)
    ...
    def test_exact_does_not_match_without_designator(self):
        ...
        "Example Charity Ministries" (no INC) should NOT match "EXAMPLE CHARITY MINISTRIES INC"
        in exact mode unless the substring literally appears.

        Actually: "example charity ministries" IS a substring of "example charity ministries inc"
        ...
        result = search_entities("example charity ministries", records, fuzzy=False)
        self.assertEqual(len(result.matches), 1)
