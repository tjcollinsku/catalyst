...
Usage:
    ...
    # results = {
    #     "persons":   [{"raw": "John A. Homan", "context": "GRANTOR: John A. Homan"}],
    #     "orgs":      [{"raw": "Do Good Ministries, Inc.", "context": "..."}],
...
#
# IRS 990 forms (especially OCR'd) produce "SECTION, A Iv" or "OH, Celina"
...
def _is_plausible_person_name(name: str) -> bool:
    ...
    # If ALL tokens are stopwords → reject (e.g. "Oh Celina", "Sign Here")
...
#
#   "GRANTOR: John A. Homan"         — labeled field, Western order
#   "HOMAN, JOHN A."                 — all-caps, inverted (last, first)
#   "John Homan"                     — no middle initial
#   "Mary Jo Winner-Baumer"          — hyphenated surname
#   "signed by John A. Homan"        — inline prose reference
#
...
# Inverted format: "HOMAN, JOHN A." — common in legal and government documents.
...
def extract_entities(text: str, doc_type: str = "OTHER") -> dict[str, list[dict[str, Any]]]:
    ...
    for m in _PERSON_INVERTED_PATTERN.finditer(text):
        ...
        # Re-order to Western format: "John A. Homan"
...
def _extract_field_after_label(text: str, label: str) -> str | None:
    ...
    _extract_field_after_label(text, "Owner") returns "DO GOOD IN HIS NAME INC"
    """
    pattern = re.compile(
...
def _split_buyer_seller(name_lines: list[str]) -> list[str]:
    ...
    The OCR produces lines like:
        ["DO GOOD IN", "HIS NAME INC", "WINNER KYLE", "J"]
        → buyer="DO GOOD IN HIS NAME INC", seller="WINNER KYLE J"
    ...
    if len(clean) == 1:
        ...
        # Common: "WINNER KYLE" / "J" / "GRIESHOP" → buyer=WINNER KYLE J, seller=GRIESHOP
        # Or: "WINNER KYLE" / "GRIESHOP" / "DOUGLAS E"
