...
class EntityExtractionPersonTests(unittest.TestCase):
    ...
    def test_extracts_labeled_grantor(self):
        text = "GRANTOR: John A. Example, of Hardin County, Ohio"
        result = self.extract(text)
        raws = [p["raw"] for p in result["persons"]]
        self.assertTrue(
            any("Example" in r for r in raws),
            f"Expected 'Example' in persons, got: {raws}",
        )

    def test_extracts_labeled_grantee(self):
        text = "GRANTEE: Example Charity RE LLC, an Ohio limited liability company"
        result = self.extract(text)
        # Org should catch the LLC, person list should be empty or not contain LLC
        raws = [p["raw"] for p in result["persons"]]
        self.assertFalse(
            any("Example Charity RE LLC" in r for r in raws),
            "LLC should not be extracted as a person",
        )

    def test_extracts_inverted_name(self):
        text = "Debtor: EXAMPLE, JOHN A., Hardin County"
        result = self.extract(text)
        raws = [p["raw"] for p in result["persons"]]
        self.assertTrue(
            any("Example" in r for r in raws),
    ...
    def test_returns_context_snippet(self):
        text = "GRANTOR: John A. Example, of Hardin County, Ohio, hereinafter"
        result = self.extract(text)
...
class EntityExtractionOrgTests(unittest.TestCase):
    ...
    def test_extracts_nonprofit_with_inc(self):
        text = "Grantor: Example Charity Ministries, Inc., an Ohio nonprofit corporation"
        result = self.extract(text)
        raws = [o["raw"] for o in result["orgs"]]
        self.assertTrue(
            any("Example Charity Ministries" in r for r in raws),
            f"Expected 'Example Charity Ministries' in orgs, got: {raws}",
        )

    def test_extracts_llc(self):
        text = "The property was transferred to Example Charity RE LLC."
        result = self.extract(text)
        raws = [o["raw"] for o in result["orgs"]]
        self.assertTrue(
            any("Example Charity RE LLC" in r for r in raws),
    ...
    def test_extracts_management_llp(self):
        text = "Secured Party: Example Hmains LLP"
        result = self.extract(text)
        raws = [o["raw"] for o in result["orgs"]]
        self.assertTrue(
            any("Example AG Management" in r for r in raws),
            f"Expected management org in orgs, got: {raws}",
        )

    def test_no_duplicate_orgs(self):
        text = (
            "Example Charity Ministries, Inc. transferred property to Example Charity Ministries, Inc."
        )
        result = self.extract(text)
        raws = [o["raw"] for o in result["orgs"]]
        do_good_count = sum(1 for r in raws if "Example Charity Ministries" in r)
...
class EntityNormalizationPersonTests(unittest.TestCase):
    ...
    def test_western_order_normalizes(self):
        self.assertEqual(self.normalize("John A. Example"), "john a example")

    def test_inverted_order_normalizes_to_same(self):
        # Both should normalize to the same canonical form
        western = self.normalize("John A. Example")
        inverted = self.normalize("EXAMPLE, JOHN A.")
    ...
    def test_strips_honorifics(self):
        self.assertEqual(self.normalize("Dr. John Example"), "john example")
        self.assertEqual(self.normalize("Mr. John Example"), "john example")

    def test_strips_jr_suffix(self):
        result = self.normalize("John Example Jr.")
        self.assertNotIn("jr", result)
        self.assertIn("john", result)
        self.assertIn("example", result)

    def test_preserves_hyphen_in_name(self):
        result = self.normalize("Jane Doe")
        self.assertIn("insider-doe", result)
...
class EntityNormalizationOrgTests(unittest.TestCase):
    ...
    def test_strips_inc_designator(self):
        with_inc = self.normalize("Example Charity Ministries, Inc.")
        without_inc = self.normalize("Example Charity Ministries")
        self.assertEqual(with_inc, without_inc)

    def test_strips_llc_designator(self):
        with_llc = self.normalize("Example Charity RE LLC")
        without_llc = self.normalize("Example Charity RE")
        self.assertEqual(with_llc, without_llc)

    def test_strips_leading_the(self):
        with_the = self.normalize("The Doe Foundation")
        without_the = self.normalize("Doe Foundation")
        self.assertEqual(with_the, without_the)

    def test_normalizes_to_lowercase(self):
        result = self.normalize("EXAMPLE CHARITY MINISTRIES INC")
...
class EntityResolutionPersonTests(TestCase):
    ...
    def test_creates_new_person_when_no_match(self):
        result = self.resolve("John A. Example", self.case)
        self.assertTrue(result.created)
        self.assertEqual(result.person.full_name, "John A. Example")

    def test_exact_match_returns_existing_person(self):
        # First call — creates
        first = self.resolve("John A. Example", self.case)
        self.assertTrue(first.created)
        # Second call with identical name — should match
        second = self.resolve("John A. Example", self.case)
    ...
    def test_fuzzy_candidate_surfaced_for_near_match(self):
        ...
        result = self.resolve("John Example", self.case)
    ...
    def test_creates_person_document_link_when_document_provided(self):
        ...
        self.resolve("John A. Example", self.case, document=doc)
        self.assertTrue(
            PersonDocument.objects.filter(
                document=doc,
                person__full_name="John A. Example",
            ).exists()
        )
...
class EntityResolutionOrgTests(TestCase):
    ...
    def test_creates_new_org_when_no_match(self):
        result = self.resolve("Example Charity Ministries, Inc.", self.case)
        self.assertTrue(result.created)

    def test_exact_match_returns_existing_org(self):
        first = self.resolve("Example Charity Ministries, Inc.", self.case)
        second = self.resolve("Example Charity Ministries, Inc.", self.case)
        self.assertFalse(second.created)
    ...
    def test_normalized_match_strips_inc_designator(self):
        ...
        second = self.resolve("Example Charity Ministries", self.case)
        self.assertFalse(second.created)
        self.assertEqual(first.org.id, second.org.id)
...
class EntityResolutionBatchTests(TestCase):
    ...
    def test_batch_creates_persons_and_orgs(self):
        extraction_result = {
            "persons": [
                {"raw": "John A. Example", "context": "GRANTOR: John A. Example"},
                {"raw": "Jane Doe", "context": "GRANTEE: Jane Doe"},
            ],
            "orgs": [
                {"raw": "Example Charity Ministries, Inc.", "context": "..."},
            ],
            "dates": [],
            "amounts": [],
            "parcels": [],
    ...
    def test_batch_matches_existing_on_second_run(self):
        extraction_result = {
            "persons": [{"raw": "John A. Example", "context": ""}],
            "orgs": [],
            "dates": [],
            "amounts": [],
            "parcels": [],
