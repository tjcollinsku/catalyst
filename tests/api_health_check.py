...
if CASE_ID:
    ...
    check("Case referrals",
          "/api/cases/{id}/referrals/", "GET", status, body, dur)
    ...
    check("Case coverage",
          "/api/cases/{id}/coverage/", "GET", status, body, dur)
    ...
    status, body, dur = api_post(
        f"/api/cases/{CASE_ID}/referrals/",
        {
            "agency_name": "IRS",
            "notes": "CSRF test referral",
        },
    )
    check(
